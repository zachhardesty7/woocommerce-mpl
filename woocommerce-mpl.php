...
class WC_MPL {
    ...
    public function plugins_loaded() {
        ...
        if (class_exists('WC_Litmos')) {
            remove_action('woocommerce_product_options_general_product_data', array(
                accessProtected(wc_litmos(), 'admin'),
                'add_simple_product_course_selection'
            ), 10);
        }
        ...
        add_action( 'woocommerce_after_order_notes',
            array($this,'add_license_field_to_checkout'));

        add_action('woocommerce_checkout_process',
            array($this,'process_license_field'));

        add_action( 'woocommerce_checkout_update_order_meta',
            array($this,'licensefield_update_order_meta'));

        add_action( 'woocommerce_admin_order_data_after_billing_address',
            array($this,'license_field_display_admin_order_meta', 10, 1));
        ...
        if (get_option('exclude_jetpack_related_from_products')) {
            add_filter('jetpack_relatedposts_filter_options', array($this,'exclude_jetpack_related_from_products'));
        }
    ...
    /**
     * adds field for entering license num on checkout
     * @param array passed by filter, icludes all fields
     * @link https://docs.woocommerce.com/document/tutorial-customising-checkout-fields-using-actions-and-filters/
     */
    function add_license_field_to_checkout($checkout) {
        echo '<div id="license-checkout-field">';

        woocommerce_form_field( 'licenseNumber', array(
            'type'          => 'text',
            'class'         => array('license-checkout-field form-row-wide'),
            'label'         => __('License Number'),
            'required'      => true,
        ), $checkout->get_value( 'licenseNumber' ));
     ...
     */
    function process_license_field() {
        // Check if set, if its not set add an error.
        if ( ! $_POST['licenseNumber'] )
            wc_add_notice( __( '<strong>License Number</strong> is a required field.' ), 'error' );
    }


    // save license num to custom fields data
    function licensefield_update_order_meta( $order_id ) {
        if ( ! empty( $_POST['licenseNumber'] ) ) {
            update_post_meta( $order_id, 'License Number', sanitize_text_field( $_POST['licenseNumber'] ) );
        }
    }


    // display custom field value on admin order page
    function license_field_display_admin_order_meta($order){
        echo '<p><strong>'.__('License Number').':</strong> ' . get_post_meta( $order->id, 'License Number', true ) . '</p>';
    }
        ...
        <div class="options_group litmos show_if_simple show_if_external">
            ...
            ?>
                <p class="form-field _wc_litmos_course_id_field">
                    <?php
                    printf('<label for="_wc_litmos_course_id[%1$b]">Litmos Course Code %2$s</label>
    ...
    public static function cartBackToVendorStorePage($vendor) {
        if (get_option('display_continue_shopping') === 'yes') {
            ...
            <div style="padding: 0 0 1.5em 0;">
                ...
                ?>" class="zh-button-left" style="font-size:18px !important;">Continue Shopping</a>
            </div>
            <?php
        }
    }
    ...
    public function productBackToVendorStorePage($vendor) {
        if (get_option('display_return_to_store') === 'yes') {
            ...
            <div style="padding: 0 0 1.5em 0;">
                <a href="<?php
                    echo get_permalink(wc_get_page_id('shop'));
                ?>" class="zh-button-left" style="font-size:18px !important;">Return to shop</a>
            </div>
            <?php
        }
    }
    ...
    public static function actionWooCheckoutUpdateOrderMeta($order_id) {
        if (get_option('constant_contact_integration') === 'yes') {
            ...
            update_post_meta($order_id, 'Subscribe to mailing list', stripslashes($_POST['our_mailing_subscribe']));
        }
    }
}
